	...
	<xsl:template match="/xmvc:root">
		...
		<html>
			...
			<body>
				...
				if( Config::Value( "logoSrc" ) )
				{
					?><img src="<?php echo( Config::Value( "logoSrc" ) ); ?>" class="logo" border="0" />
					<?php
				}
				...
				?><form method="post" action="<?php echo( count( $_GET ) ? "?" . http_build_query( $_GET, "", "&amp;" ) : "" ); ?>">
					...
					if( Config::Value( "version" ) || Config::Value( "releaseDate" ) || Config::Value( "administratorEmail" ) )
					{
						?><div class="footer">
							<?php

							if( Config::Value( "version" ) || Config::Value( "releaseDate" ) )
							{
								...
								if( Config::Value( "version" ) )
								{
									...
									echo( Config::Value( "version" ) );

									if( Config::Value( "releaseDate" ) )
									{
										?> — <?php
									}
								}
							...
							if( Config::Value( "administratorEmail" ) )
							{
								?><p class="help">Comments? Suggestions? Need assistance?<br />Contact the <a href="mailto:<?php echo( Config::Value( "administratorEmail" ) ); ?>">Administrator</a>.</p>
								<?php
							}
