<xsl:stylesheet version="1.0"
			...
			<input type="{ @type }" id="{ @name }" name="{ @name }" class="{ @name } { @type }">
				<xsl:choose>
					...
					<xsl:when test="wd:value">
						<xsl:attribute name="value"><xsl:value-of select="wd:value[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ]" /></xsl:attribute>
					</xsl:when>
				</xsl:choose>
			</input>
	...
	<xsl:template match="wd:form//wd:field[ @type = 'textarea' ]" priority="0">
		...
		<label for="{ @name }" class="{ @name } { @type }">
			...
			<textarea id="{ @name }" name="{ @name }" class="{ @name } { @type }" rows="{ @rows }" cols="{ @cols }"><xsl:choose>
				...
				<xsl:when test="wd:value"><xsl:value-of select="wd:value[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ]" /></xsl:when>
			</xsl:choose></textarea>
			<xsl:apply-templates select="wd:label[ @position = 'after' ]" mode="lang-check" />
			<xsl:apply-templates select="wd:info" mode="lang-check" />
			<xsl:apply-templates select="wd:constraint" mode="lang-check" />
	...
	<xsl:template match="wd:form//wd:field[ @type = 'submit' or @type = 'reset' or @type = 'button' ]" priority="0">
		<input type="{ @type }" name="{ @name }" id="{ @name }" class="{ @name } { @type }">
			<xsl:if test="wd:label[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ]">
				<xsl:attribute name="value"><xsl:value-of select="wd:label[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ]" /></xsl:attribute>
			</xsl:if>
		</input>
		<xsl:apply-templates select="wd:constraint" mode="lang-check" />
	</xsl:template>
			...
			<input type="{ $type }" id="{ $name }-{ $position }" name="{ $name }[]" class="{ $name } { $type }">
				<xsl:if test="wd:value">
					...
					<xsl:choose>
						...
						<xsl:when test="contains( //xmvc:strings/xmvc:*[ @key = $name ], concat( '|', wd:value[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ], '|' ) )">
							<xsl:attribute name="checked">checked</xsl:attribute>
						</xsl:when>
						<xsl:otherwise />
					</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="$type = 'select'">
						<xsl:choose>
							...
							<xsl:when test="//xmvc:strings/xmvc:*[ @key = $name ] = wd:value[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ]">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:when>
							<xsl:otherwise />
						</xsl:choose>
					...
					<xsl:otherwise>
						<xsl:if test="contains( //xmvc:strings/xmvc:*[ @key = $name ], concat( '|', wd:value[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ], '|' ) )">
							<xsl:attribute name="selected">selected</xsl:attribute>
						</xsl:if>
					</xsl:otherwise>
				</xsl:choose>
	...
	<xsl:template match="wd:form//wd:field//wd:option-group" priority="0">
		<optgroup>
			<xsl:if test="wd:label[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ]">
				<xsl:attribute name="label"><xsl:value-of select="wd:label[ php:function( 'Modules\Language\Libraries\Language::XSLTLang', $lang, (ancestor-or-self::*/@xml:lang)[last()] ) ]" /></xsl:attribute>
			</xsl:if>
